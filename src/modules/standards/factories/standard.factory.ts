...
export class StandardFactory {
   ...
   */
  createFromDto(dto: CreateStandardDto, level: number = 1): StandardEntity {
    ...
    standard.order = dto.order
    standard.level = level
    standard.isAuditable = dto.isAuditable ?? true
    standard.isActive = dto.isActive ?? true
    return standard
  }
  ...
  updateFromDto(
    ...
    if (dto.isActive !== undefined) {
      standard.isActive = dto.isActive
    }
